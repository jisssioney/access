...
class Sessions:
    ...
    do 挂起（args 须为 None，否则 ValueError）先老化：未知 sid 抛
    KeyError、非在线抛 StateError；置挂起、期限 0 并释址退租。do 恢复
    （args 为 (pool, password) 二元 tuple，非 tuple 抛 TypeError、长度错
    抛 ValueError，两字段沿用凭据约束）依次定位 sid、查停用与后端、老化、
    查池与状态、认证、取址：未知 sid 或池抛 KeyError，停用或认证非 ok 抛
    AuthError，后端故障抛 BackendError，非挂起抛 StateError，池耗尽演练、
    静态址占用或动态址耗尽抛 ResourceError；取用户在该池的静态址，否则取
    池内最小动态址，置在线，期限=now_ms+idle_ms、租期=now_ms+lease_ms；
    失败仅保留老化、认证计数与退避，不半分配。二者返回 do 格式的 LF 尾
    JSON，键序会话/状态/时刻/期限/池/地址/租期，会话/状态/池/地址为 str、
    余为 int，挂起时池/地址为空串、租期 0；缓存与审计沿用 do，同型同参
    重放无副作用、异参 ValueError。时/辅空 O(S+A)/O(1)。
    """
    ...
    def do(self, key, op, sid, args, now_ms):
        """执行一次建立/续租/下线/迁移/接管/挂起/恢复操作，返回 LF 结尾的
        JSON 字符串。
    ...
    def _suspend(self, sid, now_ms):
        ...
        挂起会话不持址、不计在线但仍占全局与单用户上限。do 已在调用前老化。
        """
        session = self._sessions.get(sid)
        if session is None:
            raise KeyError(f"unknown sid: {sid!r}")
        ...
        session["deadline"] = 0
        self._release(session)
        return self._render_session(sid, _STATE_SUSPENDED, now_ms, 0, "", "", 0)

    def _resume(self, sid, pool_id, password, now_ms):
        """挂起→在线：自指定池取用户静态址或最小动态址，期限/租期自此刻起算。

        do 已在调用前定位 sid（未知 KeyError）、查停用与后端并老化；此处依次：
        未知池 KeyError、非挂起 StateError、认证非 ok AuthError、池耗尽演练/
        静态址占用/动态址耗尽 ResourceError。租约登记与状态翻转移至全部校验
        通过之后，失败仅保留老化、认证计数与退避，不留半分配。
        """
        session = self._sessions.get(sid)
        if session is None:
            raise KeyError(f"unknown sid: {sid!r}")
        ...
        # 耗尽演练：注入期该池视为无址可分配，先于任何池变更，无半分配。
        if self._pool_is_exhausted(pool_id, now_ms):
            raise ResourceError(f"pool {pool_id!r} exhausted")
        ip_int = pool.static.get(user)
        if ip_int is None:
            # 非静态用户：取池内最小动态空闲址。
            if not pool.free:
    ...
    @staticmethod
    def _render_session(sid, state, now_ms, deadline, pool_id, address, lease):
        # 挂起/恢复的 do 格式：键序会话、状态、时刻、期限、池、地址、租期；
        # 会话/状态/池/地址为 str，余为 int；挂起时池/地址为空串、租期为 0。
        payload = {
            "会话": sid,
            "状态": state,
            "时刻": now_ms,
